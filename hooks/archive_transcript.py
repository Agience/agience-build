...
#: No ceiling, deliberately: "there can be no max chars or cap or constraints". This was 800_000,
#: then 120_000, and both numbers were the same mistake: a cap on what a session is allowed to
#: remember, chosen to fit what the store was slow at writing.
#:
#: The slowness was never storage and never length. `pipeline_unified` was feeding the whole body
#: to the encrypted lexical index, and `sse/posting.py` maintains each blind token by
#: read-modify-write, so one term cost O(artifacts already carrying it) and a body contributed
#: thousands of them. The lexical arm now indexes the offer — title, description, tags — and the
#: body goes only to the semantic arm. Measured on 71/home (2.9M vertices) after that change:
#:
#:     4KB of prose     6.0s          120KB of prose   4.9s
#:
#: Flat. A transcript's length is no longer something the store charges for, so there is nothing
#: left for a ceiling to protect and no honest number to set it to. Truncating a session's record
#: to fit a budget that does not exist would be discarding the one artifact here that cannot be
#: reconstructed from anything else.
_MAX_CHARS = None

#: And no debounce: a Stop write once cost ~32s whatever it carried, so writing often was the
#: expensive part, and 10_000 chars was the toll a turn had to pay to be recorded. With the body
#: out of the lexical index that premise is gone, and what remains is a hook that records the
#: session as it happens — which is what it was for. A turn that adds one sentence is still a
#: turn that happened.
_MIN_GROWTH_CHARS = 0

#: A transcript is `text/markdown`; minting a dedicated type for it was a mistake, reverted.
#:
...
#: added since. A cache, not a source of truth: losing this file costs one redundant write,
#: because the identity of the artifact does not live here — unlike an id index, where losing it
#: would cost a permanent duplicate, because the identity did.
#:
...
def _cap(text: str) -> str:
    """The transcript, whole — unless someone sets `_MAX_CHARS` back to a number.

    Kept as a function rather than deleted because the SHAPE of a truncation is the part worth not
    re-deriving: if a ceiling ever returns it has to keep both ends, since the opening says what was
    being attempted and the close says how it landed, and a plain head-cut keeps the question while
    throwing away the answer.

    `None` is the default and means no ceiling — see the constant for why there is no longer a
    number that could honestly go here.
    """
    if _MAX_CHARS is None or len(text) <= _MAX_CHARS:
        return text
    head = _MAX_CHARS * 2 // 5
...
def main() -> int:
    ...
    # A silent exit here is why this hook could look like it had never run at all: no artifact,
    # no log line, nothing to distinguish "declined to store" from "never invoked". A best-effort
    # hook still has to say what it decided.
    ...
    #
    # What this replaces is worth recording, because it was measured rather than theorised: an id
    # living in a local index with no lock around read → create → write lets two `Stop`
    # firings close together both read "not tracked", both create, and one overwrite the
    ...
    if incremental:
        try:
            ...
            last_size = 0
        # Not a threshold — idempotence. `_MIN_GROWTH_CHARS` is 0, so any new text is worth a
        # write; what is skipped is a turn that added nothing, where the write would store a body
        # byte-identical to the one already there. Raising this above 0 puts a toll back on being
        # recorded, which is the thing that was removed.
        if last_size and len(body) - last_size <= _MIN_GROWTH_CHARS:
            log_event("archive_transcript", outcome="skipped", hook_event=event, reason="nothing-new",
